...
export async function POST(req: NextRequest) {
  try {
    ...
    if (job.status === 'done' || job.status === 'error') return NextResponse.json({ ok: true })
    ...
    // Step 4: Upload
    await updateJob(jobId, { status: 'uploading', progress: 'Se încarcă fișierul...', progressPct: 95 })
    ...
    console.error('[generate-single-work]', message)
    await updateJob((await req.json()).jobId, { status: 'error', error: message }).catch(() => {})
